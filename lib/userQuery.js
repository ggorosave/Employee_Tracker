...
class UserQuery {
    ...
    async getManagerId(manager) {
        ...
        const firstName = await manager.split(' ')[0];
        const lastName = await manager.split(' ')[1];
        
        const managerId = await result.filter(obj => obj.first_name === firstName && obj.last_name === lastName)[0].id;
    ...
    async updateEmployeeRole(newRoleId, newManagerId, employeeId, employee) {
        const sql = `UPDATE employee SET role_id = ?, manager_id = ? WHERE id = ?`;
        const params = [newRoleId, newManagerId, employeeId]
    ...
    async updateEmployeeRolePrompt() {
        ...
        return inquirer
            .prompt([
                ...
                // destructures answer
                const { employee, newRole, newManager } = ans;

                const roleId = await this.getRoleId(newRole);

                const newManagerId = await this.getManagerId(newManager);

                const employeeId = await this.getEmployeeId(employee);

                // calls the addToDatabase function and sets department name as params
                this.updateEmployeeRole(roleId, newManagerId, employeeId, employee);
            })
    }
